...
export default function ProductFilter({ products }) {
  ...
  useEffect(() => {
    if (!products.status || !products.data || products.data.length === 0)
      return;
  ...
  const getProgressStyle = () => {
    ...
    const left =
      ((minPrice - fixedMinPrice) / (fixedMaxPrice - fixedMinPrice)) * 100;
    const right =
      100 -
      ((maxPrice - fixedMinPrice) / (fixedMaxPrice - fixedMinPrice)) * 100;
      ...
      {/* <div className="card categories-card brnads mb-3">
        ...
        <div className="list-group">
          ...
          <ul className="list-unstyled mb-0">
            {data.map((item, index) => (
              <li key={index}>
                <Link to={`/product/brands/${item?.image?.slug}`}>
                  {item.name}
                </Link>
              </li>
            ))}
      ...
      <div className="card categories-card mb-3">
        ...
        <div className="list-group">
          <div className="d-flex filterrange_box">
            <div
              className="wrapper"
              style={{ filter: `blur(${fixedMinPrice === 0 ? "3" : "0"}px)` }}
            >
              {fixedMinPrice === 0 ? (
